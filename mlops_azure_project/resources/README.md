...
The databricks CLI bundles top level is defined by file `mlops_azure_project/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`mlops_azure_project/resources/model-workflow-resource.yml`)
 - batch inference workflow (`mlops_azure_project/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`mlops_azure_project/resources/monitoring-resource.yml`)
 - feature engineering workflow (`mlops_azure_project/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`mlops_azure_project/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `mlops_azure_project/databricks.yml` and `mlops_azure_project/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops_azure_project-model     | /dev-mlops_azure_project-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest defaul_test_branch content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops_azure_project-model | /staging-mlops_azure_project-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest default_release_branch content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops_azure_project-model    | /prod-mlops_azure_project-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the defaul_test_branch branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops_azure_project-model    | /test-mlops_azure_project-experiment    |
...
You can open a PR (pull request) to modify ML code or the resource config against defaul_test_branch branch.
...
Upon merging a PR to the defaul_test_branch branch, the defaul_test_branch branch content will be deployed to the staging workspace with `staging` environment resource configurations.
...
2. Complete the `TODO` in `mlops_azure_project/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Open a pull request to merge the pushed branch into the `defaul_test_branch` branch.
...
Once CI passes, merge the PR into the `defaul_test_branch` branch. This will deploy an initial set of Databricks resources to the staging workspace.
resources will be deployed to the prod workspace on pushing code to the `default_release_branch` branch.
...
Following this, update the batch_inference_job base parameters in `mlops_azure_project/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `mlops_azure_project/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-mlops_azure_project-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-mlops_azure_project-batch-inference-job`
that runs the notebook under `mlops_azure_project/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-mlops_azure_project-batch-inference-job`.
...
`test-mlops_azure_project-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops_azure_project-batch-inference-job`.
...
After setting up CI/CD, PRs against the defaul_test_branch branch will trigger CI workflows to run unit tests, integration test and resource validation.
...
After merging a PR to the defaul_test_branch branch, continuous deployment automation will deploy the `staging` resources to the staging workspace.

When you about to cut a release, you can create and merge a PR to merge changes from defaul_test_branch to default_release_branch. Continuous deployment automation will deploy `prod` resources to the prod workspace.
