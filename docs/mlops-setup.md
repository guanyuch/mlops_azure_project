...
```
git init --initial-branch=defaul_test_branch
```
...
Commit the current `README.md` file and other docs to the `defaul_test_branch` branch of the repo, to enable forking the repo:
```

git add README.md docs .gitignore mlops_azure_project/resources/README.md
git commit -m "Adding project README"

git push upstream defaul_test_branch
```
...
* You must be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-830292400663869.9.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-830292400663869.9.azuredatabricks.net#setting/accounts). If
...
If you want to use different catalog names, please update the target names declared in the[mlops_azure_project/databricks.yml](../mlops_azure_project/databricks.yml) file.
...
The workflow will create a pull request with all the changes against your defaul_test_branch branch. Review and approve it to commit the files to deploy CI/CD for the project. 
...
After the pull request merges, pull the changes back into your local `defaul_test_branch`
branch:

```
git checkout defaul_test_branch
git pull upstream defaul_test_branch
```

## Create release branch
Create and push a release branch called `default_release_branch` off of the `defaul_test_branch` branch of the repository:
```
git checkout -b default_release_branch defaul_test_branch
git push upstream default_release_branch
git checkout defaul_test_branch
```

Your production jobs (model training, batch inference) will pull ML code against this branch, while your staging jobs will pull ML code against the `defaul_test_branch` branch. Note that the `defaul_test_branch` branch will be the source of truth for ML resource configs and CI/CD workflows.

For future ML code changes, iterate against the `defaul_test_branch` branch and regularly deploy your ML code from staging to production by merging code changes from the `defaul_test_branch` branch into the `default_release_branch` branch.

## Deploy ML resources and enable production jobs
Follow the instructions in [mlops_azure_project/resources/README.md](../mlops_azure_project/resources/README.md) to deploy ML resources
and production jobs.
...
[ML resource config guide](../mlops_azure_project/resources/README.md)  to propose, test, and deploy
